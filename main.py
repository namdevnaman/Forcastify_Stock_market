...
# df_train = df_train[['Date', 'Close']].copy()
df_train.columns = ['ds', 'y']  # Rename to Prophet-required format
df_train['ds'] = pd.to_datetime(df_train['ds'])  # Ensure 'ds' is datetime
df_train['y'] = pd.to_numeric(df_train['y'], errors='coerce')  # Ensure 'y' is numeric
df_train = df_train.dropna()  # Drop rows where y couldn't be converted
model = Prophet()
model.fit(df_train)
...
if selected_tab == "Dataframes":
    ...
    cols_to_drop = [col for col in ['Adj Close', 'Volume'] if col in data.columns]
